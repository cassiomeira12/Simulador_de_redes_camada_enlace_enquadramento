...
public class Camadas extends AnchorPane {
  ...
  private TitledPane camadaEnlace;//Camada Fisica
  private TextArea enlaceTextArea;//Area de Texto da Camada Fisica
  ...
  *********************************************/
  public Camadas(String tipo) {
    ...
    ********************************************/
    this.camadaEnlace = new TitledPane();
    this.camadaEnlace.setText("Camada Enlace de Dados " + tipo);
    this.enlaceTextArea = new TextArea();
    this.enlaceTextArea.setEditable(false);//Desativa a edicao
    this.enlaceTextArea.setFocusTraversable(false);//Desabilita o foco inicial
    this.camadaEnlace.setContent(enlaceTextArea);
    this.camadas.getPanes().add(camadaEnlace);//Adicionando no Acordeon
  ...
  *********************************************/
  ...
  /*********************************************
  * Metodo: expandirCamadaEnlace
  * Funcao: Expande a tela onde aparece as informacoes da Camada Enlace
  * Parametros: void
  * Retorno: void
  *********************************************/
  public void expandirCamadaEnlace() {
    Platform.runLater(new Runnable(){
      @Override
      public void run() {
        try {
          camadas.setExpandedPane(camadaEnlace);
          setaImagem.setLayoutY(37);
          setaImagem.setVisible(true);
        } catch (Exception e) {
          System.out.println("[ERRO] - Adicionar texto na Camada Enlace de Dados");
        }
      }
    });
  }
  ...
  /*********************************************
  * Metodo: camadaEnlace
  * Funcao: Exibe um texto na Area da Camada Enlace
  * Parametros: texto : String
  * Retorno: void
  *********************************************/
  public void camadaEnlace(String texto) {
    Platform.runLater(new Runnable(){
      @Override
      public void run() {
        try {
          String textoAntigo = enlaceTextArea.getText();
          enlaceTextArea.setText(textoAntigo + texto);
          enlaceTextArea.appendText("");
        } catch (Exception e) {
          System.out.println("[ERRO] - Adicionar texto na Camada Aplicacao");
        }
      }
